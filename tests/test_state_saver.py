...
class FakeStateSaver(StateSaverMixin):
    def __init__(self, config_section_name: str, runtime_state_groups, data_dir: Path):
...
def group_defs():
    ...
    RuntimeStateOptionDefinition(
        rsg, "last_value", "Last Value", str, "The last value", default_value="default"
    )
    RuntimeStateOptionDefinition(
        rsg, "counter", "Counter", int, "A counter", default_value=0
    )
    RuntimeStateOptionDefinition(
        rsg, "flag", "Flag", bool, "A flag", default_value=False
    )
    return [rsg]
...
class TestStateSaverMixin:
    ...
    def test_save_values_multiple_groups(self, tmp_path):
        ...
        opt1 = RuntimeStateOptionDefinition(
            rsg1, "a", "A", str, "desc", default_value="x"
        )
        rsg2 = RuntimeStateGroup("g2.ini")
        opt2 = RuntimeStateOptionDefinition(
            rsg2, "b", "B", str, "desc", default_value="y"
        )
        saver = FakeStateSaver("Section", [rsg1, rsg2], tmp_path)
    ...
    def test_state_saver_group_save_values(self, tmp_path):
        ...
        opt = RuntimeStateOptionDefinition(
            rsg, "val", "Val", str, "desc", default_value="x"
        )
