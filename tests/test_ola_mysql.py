...
class TestIsValidEvent:
    def test_valid_when_event_exists_and_correct_type(self):
        mock_conn = MagicMock()
        with patch('utils.ola_mysql.get_event', return_value={'eventForm': 'RelaySingleDay'}):
    ...
    def test_invalid_when_wrong_type(self):
        mock_conn = MagicMock()
        with patch('utils.ola_mysql.get_event', return_value={'eventForm': 'IndSingleDay'}):
...
class TestIsValidEventRace:
    def test_valid_when_in_list(self):
        mock_conn = MagicMock()
        with patch('utils.ola_mysql.get_event_races', return_value=[
            {'eventRaceId': 1}, {'eventRaceId': 2}, {'eventRaceId': 3},
        ]):
...
class TestAreValidEventRaceControlIds:
    def test_valid_when_all_in_list(self):
        mock_conn = MagicMock()
        with patch('utils.ola_mysql.get_event_race_split_time_controls', return_value=[
            {'ID': 101}, {'ID': 102}, {'ID': 103},
        ]):
            assert are_valid_event_race_control_ids(mock_conn, 565, True, 1, [101, 102]) is True
    ...
    def test_invalid_when_some_missing(self):
        ...
        with patch('utils.ola_mysql.get_event_race_split_time_controls', return_value=[
            ...
            assert are_valid_event_race_control_ids(mock_conn, 565, True, 1, [101, 102]) is False
    ...
    def test_returns_selection(self):
        ...
        with patch('utils.ola_mysql.connect', return_value=mock_conn), \
             patch('utils.ola_mysql.get_events', return_value=[
                 {'eventId': 1, 'name': 'E1', 'eventForm': 'Relay', 'startDate': '2024-01-01', 'finishDate': '2024-01-01'},
             ]):
...
class TestSelectEventRace:
    def test_returns_selection(self):
        mock_conn = MagicMock()
        with patch('utils.ola_mysql.connect', return_value=mock_conn), \
             patch('utils.ola_mysql.get_event_races', return_value=[
                 {'eventRaceId': 10, 'name': 'R1', 'raceLightCondition': 'Day', 'raceDistance': '5km', 'raceDate': '2024-01-01'},
             ]):
