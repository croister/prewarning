...
class TestFetchPunches:
    ...
    def test_applies_control_code_filter(self):
        ...
        with patch(f'{MODULE}.urlopen', return_value=resp):
            punches = _fetch_punches('http://example.com', 'u1', 0, control_codes=['101', '103'])
    ...
    def test_includes_query_parameters(self):
        ...
        with patch(f'{MODULE}.urlopen', return_value=resp) as mock_urlopen:
            _fetch_punches('http://example.com', 'unit42', 99, from_date='2024-01-15', from_time='10:00:00')
    ...
    def test_raises_http_error(self):
        ...
        resp.read.side_effect = HTTPError('http://example.com', 404, 'Not Found', {}, None)
...
class TestVerifyLastId:
    def test_success_with_punches(self):
        with patch('punchsources.punch_source_olresultat_se._fetch_punches', return_value=[{'id': '1'}]):
            result = _verify_last_id('http://ex.com', 'u1', 0)
            assert result.status is True
            assert '1 Punches' in result.message

    def test_success_no_punches(self):
        with patch('punchsources.punch_source_olresultat_se._fetch_punches', return_value=[]):
            result = _verify_last_id('http://ex.com', 'u1', 0)
            assert result.status is True
            assert 'No Punches' in result.message

    def test_catches_exception(self):
        with patch('punchsources.punch_source_olresultat_se._fetch_punches',
...
class TestVerifyDateTime:
    def test_success_with_punches(self):
        with patch('punchsources.punch_source_olresultat_se._fetch_punches', return_value=[{'id': '1'}]):
            result = _verify_date_time('http://ex.com', 'u1', 0, '2024-01-01', '10:00:00')
            assert result.status is True
            assert '1 Punches' in result.message
...
class TestVerifyControlCodes:
    def test_success_with_punches(self):
        with patch('punchsources.punch_source_olresultat_se._fetch_punches', return_value=[{'id': '1'}]):
            result = _verify_control_codes('http://ex.com', 'u1', 0, '2024-01-01', '10:00:00', ['101'])
            assert result.status is True
            assert '1 Punches' in result.message

    def test_error_when_control_codes_none(self):
        result = _verify_control_codes('http://ex.com', 'u1', 0, '2024-01-01', '10:00:00', None)
        assert result.status is False
...
class TestPunchSourceOlresultatSe:
    def test_fetch_punches_logs_unexpected_exception(self):
        with patch('punchsources.punch_source_olresultat_se.Config'), \
             patch('punchsources.punch_source_olresultat_se.urlopen', side_effect=KeyError('test_key')):
