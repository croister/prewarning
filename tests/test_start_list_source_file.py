...
class TestSelectStartListFile:
    def test_returns_selection_when_file_selected(self, wx_app):
        with patch('startlistsources.start_list_source_file.select_file',
                   return_value='/some/path/file.xml') as mock_select:
    ...
    def test_uses_prev_file_dir_as_default(self, wx_app):
        with patch('startlistsources.start_list_source_file.select_file') as mock_select:
...
class TestReadStartList:
    ...
    def test_reads_zip_file(self):
        ...
        with patch('startlistsources.start_list_source_file.ZipFile', return_value=mock_zip):
...
class TestVerifyStartListFile:
    ...
    def test_resolves_relative_path(self):
        with patch('startlistsources.start_list_source_file._read_start_list',
    ...
    def test_returns_error_when_no_teams(self):
        with patch('startlistsources.start_list_source_file._read_start_list',
                   return_value=({}, {}, {})):
            result = _verify_start_list_file(DEFAULT_START_LIST_FILE_FOLDER / 'test.xml')
            assert result.status is True
