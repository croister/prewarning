...
class TestHelpDialog:
    ...
    def test_with_hotkey_bindings(self, wx_app):
        ...
        hk1 = HotKeyBindingDefinition(
            name="test", hotkey=hotkey1, handler=lambda: None, description="Test action"
        )
        bindings = [hk1]
    ...
    def test_hotkey_with_alternates(self, wx_app):
        ...
        hk_binding = HotKeyBindingDefinition(
            name="test",
            hotkey=hk1,
            handler=lambda: None,
            description="My action",
            alternate_hotkeys=[alt1],
        )
    ...
    def test_hidden_hotkeys_not_shown(self, wx_app):
        ...
        binding_visible = HotKeyBindingDefinition(
            name="v", hotkey=hk_visible, handler=lambda: None, description="Visible"
        )
        binding_hidden = HotKeyBindingDefinition(
            name="h",
            hotkey=hk_hidden,
            handler=lambda: None,
            description="Hidden",
        ...
        dlg = HelpDialog(
            None, "1.0.0", hotkey_bindings=[binding_visible, binding_hidden]
        )
        html = self._get_html(dlg)
        assert html is not None
        text = html.ToText()
