...
class TestConfigOptionDefinition:
    ...
    def test_full_init(self):
        valid_values = [1, 2]
        d = ConfigOptionDefinition('opt2', 'Option 2', int, 'An int', True, 1, valid_values)
        assert d.name == 'opt2'
    ...
    def test_bool_cannot_have_valid_values(self):
        with pytest.raises(ValueError, match='bool can not have valid values'):
            ConfigOptionDefinition('b', 'B', bool, 'desc', default_value=True, valid_values=[True, False])

    def test_bool_cannot_have_valid_values_gen(self):
        with pytest.raises(ValueError, match='bool can not have valid values'):
            ConfigOptionDefinition('b', 'B', bool, 'desc', default_value=True, valid_values_gen=lambda: [True, False])
    ...
    def test_get_valid_values_from_gen(self):
        d = ConfigOptionDefinition('x', 'X', str, 'desc', valid_values_gen=lambda: ['c', 'd'])
    ...
    def test_get_valid_values_both_set_raises(self):
        with pytest.raises(ValueError, match='Both valid_values and valid_values_gen'):
            ConfigOptionDefinition('x', 'X', str, 'desc', valid_values=['a'], valid_values_gen=lambda: ['b'])
    ...
    def test_validate_conversion_error(self):
        ...
        assert len(errors) == 1
        assert 'expected to have the type' in errors[0] or 'invalid' in errors[0].lower()
    ...
    def test_validate_validator_returns_true(self):
        d = ConfigOptionDefinition('x', 'X', str, 'desc', validator=lambda v: VerificationResult('ok', status=True))
    ...
    def test_get_value_path_fallback_default(self, config_with_section):
        d = ConfigOptionDefinition('p', 'P', Path, 'desc', default_value=Path('/default'))
    ...
    def test_get_value_str_with_none_default_no_value_in_section(self, config_with_section):
        d = ConfigOptionDefinition('opt', 'Opt', str, 'desc', default_value=None)
        assert d.get_value(config_with_section) is None
    ...
    def test_get_value_str_value_as_empty_string_returns_none(self, config_with_section):
        d = ConfigOptionDefinition('opt', 'Opt', str, 'desc', default_value=None)
        config_with_section['opt'] = ''
        assert d.get_value(config_with_section) is None
    ...
    def test_is_enabled_enabled_by_true(self, config_with_section):
        parent = ConfigOptionDefinition('parent', 'Parent', bool, 'desc', default_value=False)
    ...
    def test_validate_validator_returns_non_verification_result(self):
        d = ConfigOptionDefinition('x', 'X', str, 'desc', validator=lambda v: 'accepted')
    ...
    def test_validate_validator_with_valid_values_gen(self):
        d = ConfigOptionDefinition('x', 'X', str, 'desc', valid_values_gen=lambda: ['x', 'y'])
        assert d.validate('x') == []
        errors = d.validate('z')
        assert len(errors) == 1
        assert 'not in the valid values' in errors[0]
    ...
    def test_validate_valid_values_override_validator(self):
        validator = MagicMock()
        d = ConfigOptionDefinition('x', 'X', str, 'desc', valid_values=['a', 'b'], validator=validator)
        d.validate('a')
    ...
    def test_validate_mandatory_with_valid_value(self):
        d = ConfigOptionDefinition('x', 'X', str, 'desc', mandatory=True, valid_values=['a'])
        assert d.validate('a') == []
...
class TestConfigSectionDefinition:
    ...
    def test_is_enabled_if_enabled_no_enabled_by(self):
        s = ConfigSectionDefinition('s', 'S', enable_type=ConfigSectionEnableType.IF_ENABLED)
        with pytest.raises(ValueError, match='not configured for the configuration section'):
            s.is_enabled({})
    ...
    def test_is_enabled_if_enabled_str_matching(self):
        ...
        section_opt = ConfigSectionOptionDefinition('other_sect', parent_opt)
        child = ConfigSectionDefinition('child', 'Child', enable_type=ConfigSectionEnableType.IF_ENABLED)
    ...
    def test_is_enabled_if_required_no_required_by(self):
        s = ConfigSectionDefinition('s', 'S', enable_type=ConfigSectionEnableType.IF_REQUIRED)
        with pytest.raises(ValueError, match='not configured for the configuration section'):
            s.is_enabled({})

    def test_is_enabled_if_required_true(self):
        requirer = ConfigSectionDefinition('requirer', 'Requirer', enable_type=ConfigSectionEnableType.ALWAYS)
        required = ConfigSectionDefinition('required', 'Required', enable_type=ConfigSectionEnableType.IF_REQUIRED)
    ...
    def test_is_enabled_if_required_false(self):
        requirer = ConfigSectionDefinition('requirer', 'Requirer', enable_type=ConfigSectionEnableType.IF_ENABLED)
        required = ConfigSectionDefinition('required', 'Required', enable_type=ConfigSectionEnableType.IF_REQUIRED)
        required.add_required_by(requirer)
        config_sections = {}
        with pytest.raises(ValueError):
...
class TestConfigVerifierDefinition:
    ...
    def test_verify_failure_with_verification_result(self, mock_config_class):
        ...
        param = ConfigSectionOptionDefinition('sect', ConfigOptionDefinition('o', 'O', str, 'd'))
