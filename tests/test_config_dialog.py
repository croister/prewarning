...
class TestConfigOptionValidator:
    ...
    def config_option(self):
        return ConfigOptionDefinition(
            "opt", "Opt", str, "desc", default_value="default"
        )
    ...
    def test_transfer_from_window(
        ...
        assert section.__setitem__.call_args_list == [call("opt", "from_ctrl")]
...
class TestConfigSectionPanelStatic:
    ...
    def test_default_button_name(self):
        assert (
            ConfigSectionPanel._default_button_name("bar") == "bar_default_button_name"
        )
    ...
    def test_too_large_for_combo_box(self, size, expected):
        assert (
            ConfigSectionPanel._too_large_for_combo_box(list(range(size))) is expected
        )
...
class TestConfigSectionPanelTracking:
    ...
    def section_def(self):
        ...
        opt = RuntimeStateOptionDefinition(
            rsg, "track", "Track", str, "desc", default_value="val"
        )
    ...
    def test_save_runtime_state(self, wx_app, section_def, config_mock):
        state_provider = MagicMock()
    ...
    def test_save_runtime_state_not_dirty(self, wx_app, section_def, config_mock):
        state_provider = MagicMock()
        frame = wx.Frame(None)
        panel = ConfigSectionPanel(
            section_def,
    ...
    def test_save_runtime_state_no_provider(self, wx_app, section_def, config_mock):
        frame = wx.Frame(None)
        panel = ConfigSectionPanel(
            section_def,
            config_mock.get_section.return_value,
    ...
    def test_save_runtime_state_no_provider_writes_file(self, wx_app, tmp_path):
        with patch("utils.config_definitions.DATA_DIR", tmp_path):
            rsg = RuntimeStateGroup("test.dat")
            opt = RuntimeStateOptionDefinition(
                rsg, "track", "Track", str, "desc", default_value="x"
    ...
    def test_validate_resets_button_colors(self, wx_app, section_def, config_mock):
        frame = wx.Frame(None)
        panel = ConfigSectionPanel(
            section_def,
            config_mock.get_section.return_value,
    ...
    def test_update_calls_dialog_when_provided(self, wx_app, section_def, config_mock):
        frame = wx.Frame(None)
        dialog = MagicMock()
        panel = ConfigSectionPanel(
            section_def,
    ...
    def test_update_skips_dialog_when_none(self, wx_app, section_def, config_mock):
        frame = wx.Frame(None)
        panel = ConfigSectionPanel(
            section_def,
            config_mock.get_section.return_value,
