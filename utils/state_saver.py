...
class _StateSaverGroup:
    ...
    def __read_config_section(self) -> SectionProxy:
        if not self._config.has_section(self.config_section_name):
            self.logger.debug(
                'The state file is missing the "%s" section, creating with default values.',
        ...
        for option_definition in self.runtime_state_group.option_definitions.values():
            if option_definition.name not in self._config[self.config_section_name]:
                self.logger.debug(
                    'The state file is missing the "%s" option in the "%s" section,'
                    " creating with default value.",
                    option_definition.name,
                    self.config_section_name,
                )
                self.__create_initial_config_option(
                    self._config[self.config_section_name], option_definition
                )
            ...
            if value is None and option_definition.default_value is not None:
                self.logger.debug(
                    'The state file is missing a value for the "%s" option in the "%s" section,'
    ...
    def __create_initial_config_option(
        self,
    ...
    def __validate(self):
        ...
        for option_definition in self.runtime_state_group.option_definitions.values():
            ...
            if len(option_validation_errors):
                self.logger.error(
                    'The state file has has the following validation errors value for the "%s" option'
                    ' in the "%s" section, using the default value.\nValidation errors:\n%s',
                    option_definition.name,
                    self.config_section_name,
                    str(option_validation_errors),
                )
                self.__create_initial_config_option(
                    self._config[self.config_section_name], option_definition
                )
    ...
    def _data_read(self, option_definition: RuntimeStateOptionDefinition) -> bool:
        ...
        """
        return (
            option_definition.name in self._data_read_state
            and self._data_read_state[option_definition.name]
        )
...
class StateSaverMixin(ABC):
    ...
    def __state_saver_groups_key(
        self, option_definition: RuntimeStateOptionDefinition
    ) -> str:
    ...
    def _data_read(self, option_definition: RuntimeStateOptionDefinition) -> bool:
        ...
        :rtype: bool
        """
        return self.state_saver_groups[
            self.__state_saver_groups_key(option_definition)
        ]._data_read(option_definition)
