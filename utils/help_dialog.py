...
class HelpDialog(wx.Frame):
    def __init__(
        ...
        contents = "<h2>Help for PreWarning {app_version}</h2>".format(
            app_version=app_version
        )
        contents += (
            "<p>The PreWarning application is intended to be used to perform pre-warning for an "
        ...
        for punch_source_name in PUNCH_SOURCES:
            ...
            contents += "<h4>{name}</h4><p>{description}</p>".format(
                name=punch_source.display_name, description=punch_source.description
            )

        contents += (
            "<h3>Start List Sources</h3>"
        ...
        for start_list_source_name in START_LIST_SOURCES:
            ...
            contents += "<h4>{name}</h4><p>{description}</p>".format(
                name=start_list_source.display_name,
