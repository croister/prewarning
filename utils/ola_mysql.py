...
def connect(
    host: str, user: str, password: str, database: str | None = None
) -> Connection:
...
def get_database_names(connection: Connection) -> List[str]:
    ...
    with connection.cursor(DictCursor) as cursor:
        ...
        logging.getLogger(LOGGER_NAME).debug(
            "Parsed database names: %s", database_names
        )
...
def get_ola_db_version(connection: Connection) -> int:
    ...
    with connection.cursor(DictCursor) as cursor:
        sql = "SELECT  `versionNumber`,  `comment`,  `moduleId` FROM `Version`;"
        cursor.execute(sql)
...
def get_event_races(connection: Connection, event_id: int) -> List[Dict[str, Any]]:
    ...
    with connection.cursor(DictCursor) as cursor:
        sql = "SELECT *  FROM `EventRaces` WHERE `eventId` = %s;"
...
def is_valid_event_race(
    connection: Connection, event_id: int, event_race_id: int
) -> bool:
...
def get_event_race_split_time_controls(
    ...
    with connection.cursor(DictCursor) as cursor:
        ...
        logging.getLogger(LOGGER_NAME).debug(
            "Event split time controls data: %s", event_split_time_controls
        )
    return event_split_time_controls


def are_valid_event_race_control_ids(
    connection: Connection,
    ola_db_version: int,
    is_relay: bool,
    event_race_id: int,
    ...
    if len(control_ids) > 0:
        ...
        valid_event_race = all(
            control_id in event_race_control_ids for control_id in control_ids
        )
        logging.getLogger(LOGGER_NAME).debug(
            "is_valid_event_race_control_ids(%s) == %s", event_race_id, valid_event_race
        )
    else:
    ...
    try:
        ...
        with connection:
            result = SelectionResult(
                caption="Databases", message="Select a OLA Database:"
            )
...
def _select_event(
    ...
    try:
        connection = connect(host, user, password, database)
        with connection:
            result = SelectionResult(caption="Events", message="Select an Event:")
...
def _select_event_race(
    host: str, user: str, password: str, database: str, event_id: int
) -> SelectionResult | bool:
    try:
        connection = connect(host, user, password, database)
        with connection:
            result = SelectionResult(
                caption="Event Races", message="Select an Event Race:"
            )
...
def get_event_race_split_times(
    ...
    with connection.cursor(DictCursor) as cursor:
        ...
        logging.getLogger(LOGGER_NAME).debug(
            "Event split times data: %s", event_split_times
        )
...
class OlaMySql(ConfigConsumer, Singleton, metaclass=_OlaMySqlMeta):
    ...
    def _connect(self):
        ...
        if not self.database:
            self.logger.error(
                'The value for "database" in the "%s" section is missing.',
