...
DOMAIN_NAME_LABEL_PATTERN = re.compile(
    r"^{label}$".format(label=DOMAIN_NAME_LABEL_PATTERN_STR)
)
...
HOSTNAME_FQDN_PATTERN = re.compile(
    r"^(?:{label}\.)+{tld}$".format(
        label=DOMAIN_NAME_LABEL_PATTERN_STR, tld=DOMAIN_NAME_TLD_PATTERN_STR
    )
)
